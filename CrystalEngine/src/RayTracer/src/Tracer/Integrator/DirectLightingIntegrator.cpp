...
namespace tracer
{
	...
	glm::vec3 DirectLightingIntegrator::Evaluate(const Ray3f& ray, const RayScene* scene,
            Sampler* sampler)
	{
		return eval_rec(ray, scene, sampler, 0, true);
	}

	glm::vec3 DirectLightingIntegrator::eval_rec(const Ray3f& ray, const RayScene* scene, Sampler* sampler,
