...
namespace crystal
{
    ...
    float GGXDistribution::D(const Vector3f& wh) const
    {
        ...
        float A = square(phi.x) / square(m_alpha.x) + square(phi.y) / square(m_alpha.y);
        return 1.f / (glm::pi<float>() * m_alpha.x * m_alpha.y * square(A - cos2Theta * (A - 1)));
    ...
    Vector3f GGXDistribution::Sample_wh(const Vector3f& wo, const Vector2f& sample) const
    {
        float cosTheta, phi;
        if (m_alpha.x == m_alpha.y)
        {
            cosTheta = std::sqrt((1 - sample.x) / (sample.x * (m_alpha.x * m_alpha.x - 1) + 1));
        ...
        {
            phi = std::atan(m_alpha.y / m_alpha.x *
            ...
            float A = square(cosPhi) / square(m_alpha.x) + square(sinPhi) / square(m_alpha.y);
    ...
    float GGXDistribution::lambda(const Vector3f& wh) const
    {
        ...
        auto phi = getCosSinPhi(wh, std::sqrt(sin2Theta));
        float alpha2 = square(phi.x) * square(m_alpha.x) + square(phi.y) * square(m_alpha.y);
    ...
    float BeckmannDistribution::D(const Vector3f& wh) const
    {
        ...
        float bot = glm::pi<float>() * m_alpha.x * m_alpha.y * cos4Theta;
        float top = std::exp(-tan2Theta * (square(v.x) / square(m_alpha.x) + square(v.y) / square(m_alpha.y)));
        return top / bot;
    }
    ...
    Vector3f BeckmannDistribution::Sample_wh(const Vector3f& wo, const Vector2f& sample) const
    {
        ...
        if (m_alpha.x == m_alpha.y)
        {
            tan2Theta = -square(m_alpha.x) * logSample;
        ...
        else
        {
            ...
            float sinPhi = glm::sin(phi), cosPhi = glm::cos(phi);
            tan2Theta = -logSample /
                (cosPhi * cosPhi / square(m_alpha.x) + sinPhi * sinPhi / square(m_alpha.y));
        }
    ...
    float BeckmannDistribution::lambda(const Vector3f& wh) const
    {
        ...
        float alpha = std::sqrt(square(v.x) * square(m_alpha.x) + square(v.y) * square(m_alpha.y));
